...
typedef struct 
{
    PyObject *f;
    PyObject *arg;
} xmlStreamReaderPyCtxt;
typedef xmlStreamReaderPyCtxt *xmlStreamReaderPyCtxtPtr;

static void 
libxml_xmlStreamReaderErrorCallback(void *arg, 
				  const char *msg,
				  int severity,
				  xmlTextReaderLocatorPtr locator)
{
    xmlStreamReaderPyCtxt *pyCtxt = (xmlStreamReaderPyCtxt *)arg;
    PyObject *list;
    PyObject *result;
    ...
    PyTuple_SetItem(list, 2, libxml_intWrap(severity));
    PyTuple_SetItem(list, 3, libxml_xmlStreamReaderLocatorPtrWrap(locator));
    result = PyEval_CallObject(pyCtxt->f, list);
    if (result == NULL)
    {
	/* TODO: manage for the exception to be propagated... */
...
PyObject *
libxml_xmlStreamReaderSetErrorHandler(ATTRIBUTE_UNUSED PyObject *self, PyObject *args)
{
    xmlTextReaderPtr reader;
    xmlStreamReaderPyCtxtPtr pyCtxt;
    ...
    if (!PyArg_ParseTuple(args, (char *)"OOO:xmlStreamReaderSetErrorHandler", &pyobj_reader, &pyobj_f, &pyobj_arg))
        return(NULL);
    reader = (xmlTextReaderPtr) PyxmlStreamReader_Get(pyobj_reader);
    /* clear previous error handler */
    xmlStreamReaderGetErrorHandler(reader,&f,&arg);
    if (arg != NULL) {
	if (f == libxml_xmlStreamReaderErrorCallback) {
	    /* ok, it's our error handler! */
	    pyCtxt = (xmlStreamReaderPyCtxtPtr)arg;
	    Py_XDECREF(pyCtxt->f);
	    Py_XDECREF(pyCtxt->arg);
	    xmlFree(pyCtxt);
	}
    ...
    xmlStreamReaderSetErrorHandler(reader,NULL,NULL);
    /* set new error handler */
    if (pyobj_f != Py_None)
    {
	pyCtxt = (xmlStreamReaderPyCtxtPtr)xmlMalloc(sizeof(xmlStreamReaderPyCtxt));
	if (pyCtxt == NULL) {
	    py_retval = libxml_intWrap(-1);
	    return(py_retval);
	}
	...
	Py_XINCREF(pyobj_arg);
	pyCtxt->arg = pyobj_arg;
	xmlStreamReaderSetErrorHandler(reader,libxml_xmlStreamReaderErrorCallback,pyCtxt);
    }
...
PyObject *
libxml_xmlStreamReaderGetErrorHandler(ATTRIBUTE_UNUSED PyObject *self, PyObject *args)
{
    xmlTextReaderPtr reader;
    xmlStreamReaderPyCtxtPtr pyCtxt;
    xmlTextReaderErrorFunc f;
    ...
    if (!PyArg_ParseTuple(args, (char *)"O:xmlStreamReaderSetErrorHandler", &pyobj_reader))
        return(NULL);
    reader = (xmlTextReaderPtr) PyxmlStreamReader_Get(pyobj_reader);
    xmlStreamReaderGetErrorHandler(reader,&f,&arg);
    py_retval = PyTuple_New(2);
...
PyObject *
libxml_xmlFreeStreamReader(ATTRIBUTE_UNUSED PyObject *self, PyObject *args) {
    xmlTextReaderPtr reader;
    PyObject *pyobj_reader;
    xmlStreamReaderPyCtxtPtr pyCtxt;
    xmlTextReaderErrorFunc f;
    ...
    if (!PyArg_ParseTuple(args, (char *)"O:xmlFreeStreamReader", &pyobj_reader))
        return(NULL);
    reader = (xmlTextReaderPtr) PyxmlStreamReader_Get(pyobj_reader);
...
#include "libxml2-export.c"
    ...
    {(char *)"xmlStreamReaderSetErrorHandler", libxml_xmlStreamReaderSetErrorHandler, METH_VARARGS, NULL },
    {(char *)"xmlStreamReaderGetErrorHandler", libxml_xmlStreamReaderGetErrorHandler, METH_VARARGS, NULL },
    {(char *)"xmlFreeStreamReader", libxml_xmlFreeStreamReader, METH_VARARGS, NULL },
...
init_libxml2addon(void)
{
    ...
    m = Py_InitModule((char *) "_libxml2addon", libxmlMethods);
    initialized = 1;
